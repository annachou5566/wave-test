...
const translations = {
    ...
    vi: {
        ...
        col_target: "DỰ ĐOÁN",
        tip_ai_title: "MÔ HÌNH ƯỚC TÍNH",
        tip_ai_disclaimer: `
            <div style="margin-bottom:8px; border-bottom:1px dashed #555; padding-bottom:6px; color:#ccc">
                <b>Cơ sở dữ liệu:</b>
            </div>
            <ul style='margin: 0; padding-left: 15px; list-style-type: circle; color:#bbb; line-height: 1.5; margin-bottom: 10px;'>
                <li><b>Công thức:</b> Tổng hợp Min Vol phiên trước và tốc độ giao dịch (Velocity) thời gian thực.</li>
                <li><b>Phạm vi:</b> Thuật toán kích hoạt trong 16 giờ cuối cùng.</li>
                <li><b>Điều chỉnh:</b> Áp dụng hệ số biến thiên dựa trên lượng người tham gia và độ sâu thị trường.</li>
            </ul>
            <div style="border-top: 1px solid #444; padding-top: 8px; margin-top: 8px;">
                <div style="color: #F6465D; font-size: 0.85em; line-height: 1.4; font-weight: 500;">
                ...
                <div style="color: #888; font-size: 0.8em; font-style: italic; line-height: 1.3; margin-top: 2px;">
                    Số liệu chỉ mang tính chất tham khảo, <b>không phải lời khuyên đầu tư</b>. Bạn tự chịu trách nhiệm hoàn toàn với quyết định giao dịch.
                </div>
            </div>`,
        // --- UPDATED ---
    ...
    zh: {
        ...
        tip_ai_disclaimer: `
            ...
            <ul style='margin: 0; padding-left: 15px; list-style-type: circle; color:#bbb; line-height: 1.5; margin-bottom: 10px;'>
                <li><b>公式：</b> 综合上一时段的最小交易量 (Min Vol) 与实时交易速度 (Velocity)。</li>
                <li><b>范围：</b> 算法仅在最后 16 小时内激活。</li>
            ...
            <div style="border-top: 1px solid #444; padding-top: 8px; margin-top: 8px;">
                ...
                <div style="color: #888; font-size: 0.8em; font-style: italic; line-height: 1.3; margin-top: 2px;">
                    数据仅供参考，<b>不构成投资建议</b>。您需对自己的交易决策完全负责。
                </div>
            </div>`,
        // --- UPDATED ---
    ...
    ko: {
        ...
        tip_ai_disclaimer: `
            ...
            <ul style='margin: 0; padding-left: 15px; list-style-type: circle; color:#bbb; line-height: 1.5; margin-bottom: 10px;'>
                <li><b>공식:</b> 이전 세션의 최저 거래량(Min Vol)과 실시간 거래 속도(Velocity)를 종합.</li>
                <li><b>범위:</b> 종료 16시간 전부터 알고리즘 활성화.</li>
                <li><b>조정:</b> 참여자 수와 시장 깊이에 따라 변동 계수 적용.</li>
            </ul>
            <div style="border-top: 1px solid #444; padding-top: 8px; margin-top: 8px;">
                <div style="color: #F6465D; font-size: 0.85em; line-height: 1.4; font-weight: 500;">
                    ⚠ 면책 조항:
                ...
                <div style="color: #888; font-size: 0.8em; font-style: italic; line-height: 1.3; margin-top: 2px;">
                    제공된 데이터는 참고용이며 <b>투자 조언이 아닙니다</b>. 모든 거래 결정에 대한 책임은 사용자 본인에게 있습니다.
                </div>
            </div>`,
        // --- UPDATED ---
...
/* ==========================================================
   RENDER MARKET HEALTH (FIX: TEXT NGẮN - SỐ FULL KHÔNG LẺ)
   ========================================================== */
   function renderMarketHealthTable() {
    ...
    const cols = [
        ...
        { key: 'min_vol',     label: 'MIN VOL',     align: 'text-center', tooltip: 'tip_min_vol' },
        ...
        { key: 'speed_match', label: 'SPD / MATCH', align: 'text-center d-none d-md-table-cell', tooltip: 'tip_speed_match' },
        { key: 'ord_spr',     label: 'ORD / SPR',   align: 'text-center d-none d-md-table-cell', tooltip: 'tip_ord_spr' },
        { key: 'target',      label: 'PREDICTION',  align: 'text-center px-2', tooltip: 'tip_ai_title' }
    ];
    ...
    cols.forEach(c => {
        ...
        let tipText = c.tooltip ? (t[c.tooltip] || '') : '';
        let labelWithTooltip = tipText 
            ? `<span data-bs-toggle="tooltip" data-bs-placement="top" title="${tipText}" style="cursor:help;">${labelText}</span>`
            : labelText;

        theadHtml += `<th class="${c.align}" onclick="toggleHealthSort('${c.key}')" style="cursor:pointer; user-select:none; vertical-align:middle;">
                        ${labelWithTooltip} <i class="fas ${icon} sort-icon ${activeClass}"></i>
                      </th>`;
    });
    theadHtml += '</tr>';
    ...
    // 3. XỬ LÝ DATA
    let activeProjects = (typeof compList !== 'undefined' ? compList : []).filter(c => {
    ...
    // --- BỘ FORMATTER CHUẨN ---
    // 1. Số đầy đủ, không thập phân (Ví dụ: 1,200,500)
    const fmtNoDec = (num) => !num ? '$0' : '$' + Math.round(num).toLocaleString('en-US');
    
    // 2. Số rút gọn cho các cột ít quan trọng (Ví dụ: 1.2M)
    ...
    activeProjects.forEach(c => {
        ...
        let tokenHtml = `
            <div class="token-cell-wrapper" style="justify-content: center; display: flex; align-items: center; gap: 8px;">
                <img src="${localImgPath}" onerror="this.onerror=null; this.src='${defaultImgPath}';" style="width:32px; height:32px; border-radius:50%; border:1px solid #333; flex-shrink:0;">
        ...
        let dateRange = '--';
        if (c.start && c.end) {
            let endObj = new Date(c.end + 'T' + (c.endTime || '23:59') + ':00Z');
            let diff = endObj - now;
            if (diff > 0) {
                let dd = Math.floor(diff / 86400000), dh = Math.floor((diff % 86400000) / 3600000), dm = Math.floor((diff % 3600000) / 60000);
                countStr = `${dd}d ${dh}h ${dm}m`;
            }
            dateRange = `${formatDateShort(c.start)} - ${formatDateShort(c.end)}`;
        }
        let durationHtml = `<div class="cell-stack justify-content-center"><span class="cell-primary text-white">${countStr}</span><span class="cell-secondary">${dateRange}</span></div>`;

        // --- WIN / POOL ---
        let winnersText = c.topWinners ? c.topWinners.replace(/\(p\d+\)/gi, '').trim() : '--';
        let qtyRaw = parseFloat(c.rewardQty) || 0;
        let winPoolHtml = `<div class="cell-stack justify-content-center"><span class="cell-primary text-white">${winnersText}</span><span class="cell-secondary">${qtyRaw.toLocaleString()} ${c.name}</span></div>`;

        // --- VAL / PRICE ---
        let price = ma.price || c.cachedPrice || 0;
        let priceValHtml = `<div class="cell-stack justify-content-center"><span class="cell-primary text-highlight">${fmtCompact(qtyRaw * price)}</span><span class="cell-secondary">$${price.toLocaleString()}</span></div>`;

        // --- RULE ---
        let rt = c.ruleType || 'buy_only'; 
        let ruleMain = rt==='trade_x4' ? t.rule_buy_sell : (rt==='trade_all' ? t.rule_buy_sell : t.rule_buy);
        let ruleClass = rt==='buy_only' ? 'rp-buy' : 'rp-all';
        let ruleSub = rt==='trade_x4' ? t.rule_limit_x4 : '&nbsp;';
        let ruleHtml = `
            <div class="cell-stack align-items-center justify-content-center">
                <div class="rule-pill ${ruleClass}">${ruleMain}</div>
                <span class="cell-secondary" style="${rt==='trade_x4'?'color:#F0B90B; font-weight:700; opacity:1':'opacity:0'}; font-size:0.65rem; margin-top:2px;">${ruleSub}</span>
            </div>`;

        // --- MIN VOL (Text: Prev - Số: Full) ---
        let h = c.history || [];
        let curTarget = h.length > 0 ? h[h.length-1].target : 0;
        let prevTarget = h.length > 1 ? h[h.length-2].target : 0; 
        
        // Dùng fmtNoDec để ra số full: 1,234,567
        let subMinVol = prevTarget > 0 ? `Prev: ${fmtNoDec(prevTarget)}` : '--'; 
        let minVolHtml = `<div class="cell-stack justify-content-center"><span class="cell-primary text-gold">${fmtNoDec(curTarget)}</span><span class="cell-secondary">${subMinVol}</span></div>`;

        // --- DAILY VOL (Text: Yest - Số: Full) ---
        let todayVol = c.real_alpha_volume || 0;
        let yestVolObj = (c.real_vol_history || []).find(x => x.date === yestStr);
        let yestVol = yestVolObj ? yestVolObj.vol : 0;
        
        // Dùng fmtNoDec để ra số full: 1,234,567
        let subDailyVol = yestVol > 0 ? `Yest: ${fmtNoDec(yestVol)}` : '--';
        ...
        // --- SPD / MATCH ---
        let realTimeVol = parseFloat(ma.realTimeVol) || 0;
        let velocity = parseFloat(ma.velocity) || 0;
        let ops = velocity > 0 ? (velocity / 60).toFixed(1) + ' ops' : '0 ops';
        
        // Hiện số tiền full cho Speed
        let matchSpdHtml = `<div class="cell-stack justify-content-center">
                                <span class="cell-primary text-white">$${Math.round(realTimeVol).toLocaleString()}</span>
                                <span class="cell-secondary">${ops}</span>
                            </div>`;

        // --- ORD / SPR ---
        let ordVal = (parseFloat(ma.avgTicket)||0) > 0 ? '$'+Math.round(ma.avgTicket).toLocaleString() : '$0';
        let sprRaw = parseFloat(ma.spread) || 0;
        let ordSprHtml = `<div class="cell-stack justify-content-center">
                              <span class="cell-primary text-white">${ordVal}</span>
                              <span class="cell-secondary ${sprRaw>1?'text-red':'text-green'}">${sprRaw.toFixed(2)}%</span>
                          </div>`;

        // --- PREDICTION ---
        let aiTargetHtml = (typeof calculateAiTarget === 'function') ? calculateAiTarget(c) : '<td class="text-center">--</td>';
        ...
        <tr style="cursor:pointer; border-bottom: 1px solid rgba(255,255,255,0.05);" onclick="jumpToCard('${c.db_id}')">
            ...
            <td class="text-center d-none d-md-table-cell">${matchSpdHtml}</td>
            <td class="text-center font-num d-none d-md-table-cell">${ordSprHtml}</td>
            ${aiTargetHtml}
        </tr>`;
    });
...
/* ==========================================================
   HÀM AI TARGET: VIÊN THUỐC 3 PHẦN (LOWER - MATCH - HIGHER)
   ========================================================== */
   function calculateAiTarget(c) {
    const lang = (typeof currentLang !== 'undefined') ? currentLang : 'en';
    const t = translations[lang] || translations['en'];

    let meta = c;
    let prediction = meta.ai_prediction || {};
    let target = prediction.target || 0;
    if (target === 0) { target = 150000; } 
    
    let formattedTarget = '$' + Math.round(target).toLocaleString('en-US');
    let isUrgent = true;

    // --- LOGIC GIẢ LẬP TỈ LỆ 3 PHẦN (LOWER | MATCH | HIGHER) ---
    let voteKey = `vote_${c.db_id}`;
    let myVote = localStorage.getItem(voteKey); // 'low', 'match', 'high'
    let myGuess = localStorage.getItem(`guess_${c.db_id}`);
    
    // Tạo 3 con số % ngẫu nhiên dựa trên ID (để nó cố định không nhảy loạn xạ)
    let seed = String(c.db_id).charCodeAt(0);
    
    // Giả lập tỉ lệ cơ bản
    let pctLow = (seed % 30) + 20;  // 20-50%
    let pctHigh = (seed % 20) + 20; // 20-40%
    let pctMatch = 100 - pctLow - pctHigh; // Phần còn lại

    // Nếu user đã vote cái nào, tăng cái đó lên để user thấy mình đã tác động
    if (myVote === 'low') { pctLow += 10; pctMatch -= 5; pctHigh -= 5; }
    else if (myVote === 'high') { pctHigh += 10; pctMatch -= 5; pctLow -= 5; }
    else if (myVote === 'match') { pctMatch += 10; pctLow -= 5; pctHigh -= 5; }

    // Đảm bảo không âm
    if(pctLow < 5) pctLow = 5; if(pctHigh < 5) pctHigh = 5; if(pctMatch < 5) pctMatch = 5;

    // Class active để sáng đèn khúc đã chọn
    let actLow = myVote === 'low' ? 'active' : '';
    let actMatch = myVote === 'match' ? 'active' : '';
    let actHigh = myVote === 'high' ? 'active' : '';

    // Tooltip nội dung
    let tooltipHtml = `<div class='ai-tooltip-panel'><div class='ai-header'><i class='fas fa-brain me-2'></i> ${t.tip_ai_title}</div><div class='ai-body'>${t.tip_ai_disclaimer}</div></div>`;

    return `
    <td class="text-center col-ai-target" style="vertical-align: middle;">
        <div class="ai-cell-wrapper">
            
            <div class="ai-value-text mb-1" 
                 data-bs-toggle="tooltip" 
                 data-bs-html="true" 
                 data-bs-custom-class="custom-ai-tooltip"
                 data-bs-title="${tooltipHtml.replace(/"/g, '&quot;')}">
                 ${isUrgent ? '<i class="fas fa-fire text-danger me-1" style="font-size:0.8rem"></i>' : ''}${formattedTarget}
            </div>

            <div class="tri-pill-container">
                
                <div class="tri-part low ${actLow}" style="width: ${pctLow}%" 
                     onclick="toggleVoteInput('${c.db_id}', 'low', this)" 
                     title="Lower (Thấp hơn)">
                     <span>${pctLow}%</span>
                </div>

                <div class="tri-part match ${actMatch}" style="width: ${pctMatch}%" 
                     onclick="toggleVoteInput('${c.db_id}', 'match', this)" 
                     title="Agree (Đồng ý)">
                     <span>${pctMatch}%</span>
                </div>

                <div class="tri-part high ${actHigh}" style="width: ${pctHigh}%" 
                     onclick="toggleVoteInput('${c.db_id}', 'high', this)" 
                     title="Higher (Cao hơn)">
                     <span>${pctHigh}%</span>
                </div>
            ...
            <div class="tri-labels">
                <small class="l-low">Lower</small>
                <small class="l-match">Agree</small>
                <small class="l-high">Higher</small>
            </div>

            <div id="vote-box-${c.db_id}" class="vote-popup" onclick="event.stopPropagation()">
                <div class="vote-header" id="vote-header-${c.db_id}">YOUR PREDICTION?</div>
                <input type="text" id="input-vote-${c.db_id}" class="popup-input" 
                       placeholder="${Math.round(target)}" value="${myGuess || ''}" 
                       onkeyup="formatCurrency(this)">
                <button class="popup-btn" onclick="confirmVote('${c.db_id}')">CONFIRM</button>
            </div>
...
/* --- CÁC HÀM XỬ LÝ VOTE MỚI --- */
function toggleVoteInput(dbId, type, el) {
    event.stopPropagation();
    document.querySelectorAll('.vote-popup').forEach(p => p.classList.remove('show'));
    
    // Lưu loại vote
    localStorage.setItem(`vote_${dbId}`, type);
    
    // Đổi tiêu đề Popup cho hợp ngữ cảnh
    let headerEl = document.getElementById(`vote-header-${dbId}`);
    let inputEl = document.getElementById(`input-vote-${dbId}`);
    
    if (headerEl) {
        if(type === 'low') headerEl.innerHTML = '<span class="text-danger">LOWER</span> THAN TARGET?';
        if(type === 'high') headerEl.innerHTML = '<span class="text-success">HIGHER</span> THAN TARGET?';
        if(type === 'match') {
            headerEl.innerHTML = '<span class="text-brand">AGREE</span> WITH TARGET?';
            // Nếu đồng ý thì điền sẵn số target vào ô input luôn cho tiện
            if(inputEl) inputEl.value = inputEl.placeholder; 
        } else {
             // Nếu chọn cao/thấp thì clear ô input để họ tự nhập (hoặc giữ cũ)
             if(inputEl && inputEl.value === inputEl.placeholder) inputEl.value = '';
        }
    }

    // Render lại để cập nhật % và màu sắc
    renderMarketHealthTable();

    // Hiện popup
    let popup = document.getElementById(`vote-box-${dbId}`);
    if(popup) popup.classList.add('show');
}


/* ==========================================================
   HÀM AI TARGET (FIX: LẤY TEXT TỪ DICTIONARY + VIÊN THUỐC)
   ========================================================== */
   function calculateAiTarget(c) {
    // 1. Lấy ngôn ngữ hiện tại để hiển thị đúng Text
    const lang = (typeof currentLang !== 'undefined') ? currentLang : 'en';
    const t = translations[lang] || translations['en'];
    ...
    // Mock data để test hiển thị (nếu chưa có dữ liệu thật)
    if (target === 0) { target = 150000; } 
    ...
    // Giả lập % cho thanh Bar (Viên thuốc)
    let pseudoSeed = String(c.db_id).charCodeAt(0);
    let upPercent = 50; 
    if (myVote === 'up') upPercent = 75;
    else if (myVote === 'down') upPercent = 25;
    else upPercent = (pseudoSeed % 40) + 30;

    let barText = `${upPercent}%`; 
    let upActive = myVote === 'up' ? 'voted-up' : '';
    let downActive = myVote === 'down' ? 'voted-down' : '';

    // --- [QUAN TRỌNG] NỘI DUNG TOOLTIP TỪ TỪ ĐIỂN ---
    // Không còn chế ra Confidence hay Method nữa
    let tooltipHtml = `
        <div class='ai-tooltip-panel'>
            <div class='ai-header'>
                <i class='fas fa-brain me-2'></i> ${t.tip_ai_title}
            </div>
            <div class='ai-body'>
                ${t.tip_ai_disclaimer}
            </div>
    ...
    // --- RETURN HTML (Đã thêm style bo tròn cho viên thuốc) ---
    return `
    <td class="text-center col-ai-target" style="vertical-align: middle;">
        <div class="ai-cell-wrapper">
            ...
            <div class="interaction-area">
                <div class="ghost-btn down ${downActive}" onclick="toggleVoteInput('${c.db_id}', 'down', this)">▼</div>

                <div class="sentiment-track" style="border-radius: 50px !important;" onclick="toggleVoteInput('${c.db_id}', '${upPercent>=50?'up':'down'}', this)">
                    <div class="sentiment-fill" style="width: ${upPercent}%; border-radius: 50px;"></div>
                    <div class="sentiment-text-overlay">${barText}</div>
                </div>

                <div class="ghost-btn up ${upActive}" onclick="toggleVoteInput('${c.db_id}', 'up', this)">▲</div>
            </div>

            <div id="vote-box-${c.db_id}" class="vote-popup" onclick="event.stopPropagation()">
                <div class="vote-header">${t.modal_pred_title || 'YOUR PREDICTION'}?</div>
                <input type="text" id="input-vote-${c.db_id}" class="popup-input" 
                       placeholder="${Math.round(target)}" value="${myGuess || ''}" 
                       onkeyup="formatCurrency(this)">
                <button class="popup-btn" onclick="confirmVote('${c.db_id}')">CONFIRM</button>
...
// 2. HÀM BẬT/TẮT POPUP (FIX LỖI BẤM KHÔNG ĂN)
function toggleVoteInput(tokenId, type, btnEl) {
    // Ngăn sự kiện lan truyền lên thẻ cha (quan trọng)
    if(event) event.stopPropagation();

    // Tắt hết popup khác đang mở
    document.querySelectorAll('.vote-popup').forEach(el => el.classList.remove('show'));

    let currentVote = localStorage.getItem(`vote_${tokenId}`);
    let box = document.getElementById(`vote-box-${tokenId}`); // Tìm popup theo ID

    // Nếu bấm lại nút đã chọn -> Hủy vote
    if (currentVote === type) {
        localStorage.removeItem(`vote_${tokenId}`);
        updateGridValuesOnly(); // Vẽ lại giao diện
        return;
    }
    ...
    // Hiện popup nhập số
    if (box) {
        box.classList.add('show'); // Thêm class show để hiển thị
        
        // Tự động focus vào ô nhập
        setTimeout(() => {
            let inp = document.getElementById(`input-vote-${tokenId}`);
            if(inp) inp.focus();
        }, 100);
    } else {
        console.error("Popup box not found for ID:", tokenId);
    }
...
// 3. HÀM XÁC NHẬN (CONFIRM)
function confirmVote(tokenId) {
    if(event) event.stopPropagation();

    let inp = document.getElementById(`input-vote-${tokenId}`);
    if (inp && inp.value) {
        localStorage.setItem(`guess_${tokenId}`, inp.value);
        
        // Show Toast thông báo (nếu hàm này có sẵn)
        if(typeof showToast === 'function') showToast(`Saved: ${inp.value}`, "success");
    }
    
    // Đóng popup
    let box = document.getElementById(`vote-box-${tokenId}`);
    if(box) box.classList.remove('show');
    
    // Vẽ lại bảng để cập nhật số liệu
    updateGridValuesOnly();
}

// Xử lý click ra ngoài để đóng Popup
document.addEventListener('click', function(e) {
    // Nếu click không trúng popup và không trúng nút bấm -> Đóng hết
    if (!e.target.closest('.vote-popup') && !e.target.closest('.ghost-btn') && !e.target.closest('.sentiment-track')) {
        document.querySelectorAll('.vote-popup').forEach(el => el.classList.remove('show'));
    }
});

// 3. HÀM XỬ LÝ CLICK VOTE
function handleVote(tokenId, type, btnElement) {
    event.stopPropagation();
    let wrapper = btnElement.closest('.sentiment-wrapper');
    let btnUp = wrapper.querySelector('button:first-child'); // Nút đầu tiên (▲)
    let btnDown = wrapper.querySelector('button:last-child'); // Nút cuối cùng (▼)
    let barFill = wrapper.querySelector('.sentiment-fill-up'); // Thanh màu

    let currentVote = localStorage.getItem(`vote_${tokenId}`);
    
    // Reset UI
    if(btnUp) btnUp.classList.remove('active-up');
    if(btnDown) btnDown.classList.remove('active-down');
    ...
    if (currentVote === type) {
        ...
        localStorage.setItem(`vote_${tokenId}`, type);
        if (type === 'up') {
            if(btnUp) btnUp.classList.add('active-up');
            if(barFill) barFill.style.width = '75%';
        } else {
            if(btnDown) btnDown.classList.add('active-down');
            if(barFill) barFill.style.width = '25%';
        }
    }
}
/* KẾT THÚC ĐOẠN CODE CẦN COPY */
    
    

// --- HÀM PHỤ ĐỂ SORT ---
function calculateAiTargetValue(token) {
    if (!token.end) return 0;
    let endStr = `${token.end}T${token.endTime || "13:00"}Z`;
    if (token.end.indexOf('T') > -1) endStr = token.end; // Nếu DB đã lưu đủ format
    const endTime = new Date(endStr).getTime();
    if ((endTime - Date.now()) > 16 * 3600 * 1000) return 0; // Hơn 16h thì coi như 0 để xếp cuối

    const ma = token.market_analysis || {};
    const volPerSec = parseFloat(ma.realTimeVol || 0); 
    let currentTotalVol = parseFloat(token.total_accumulated_volume || 0);
    if (currentTotalVol === 0 && token.real_vol_history) {
         currentTotalVol = token.real_vol_history.reduce((a,b)=>a+parseFloat(b.vol||0),0) + parseFloat(token.real_alpha_volume||0);
    }
    const diffHours = (endTime - Date.now()) / (1000 * 60 * 60);
    let secondsLeft = Math.max(0, diffHours * 3600);
    const finalProjectedVol = currentTotalVol + (volPerSec * secondsLeft * 1.05);

    const rule = (token.ruleType || "buy_only").toLowerCase(); 
    let k = 0.38; 
    // ... (Giản lược logic K để sort nhanh, lấy K thấp nhất cho an toàn) ...
    // Hoặc bạn copy logic K đầy đủ vào đây nếu muốn sort chính xác tuyệt đối.
    if (rule.includes('buy')) k = 1.2; 
    if (rule.includes('x4')) k = 0.5;

    let ruleMult = rule.includes('x4') ? 4.0 : 1.0;
    const winners = parseInt(token.topWinners || 5000);
    return (finalProjectedVol * ruleMult) / (winners * k);
}
